...
class ScreenHandler:
    def __init__(self, config):
        ...
        print('[ScreenHandler] Initialized')

    ''' image출력, 검출된객체 박스 그리기 '''    
    def draw_screen(self, image, detections):
        '''
        ...
        if detections is not None:
            for det in detections:
                ...
                cv2.putText(frame, f'{class_label}', (x1, y1-10), cv2.FONT_HERSHEY_SIMPLEX, color, 0.5, 2)
    ...
    def inverse_transform(self, box_norm):
        ...
        x1 = max(0.0, min(x1, self.view_w))
        y1 = max(0.0, min(y1, self.view_h))
        x2 = max(0.0, min(x2, self.view_w))
        y2 = max(0.0, min(y2, self.view_h))
